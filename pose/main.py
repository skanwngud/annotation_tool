...
async def pose(inp: Input):
    ...
    for img_info in inp.images:
        ...
        res = model(src, conf=inp.conf, verbose=False)[0]
        bbox = res.boxes.data.cpu().numpy().tolist()
        kpts = res.keypoints.data.cpu().numpy().tolist()[0]

        results.update({img_info["name"]: {"bbox": bbox, "kpts": kpts}})
